...
public class RemoteInjectorService extends IRemoteInjector.Stub {
    ...
    private final int touchId; // single touch pointer ID (0-2)
        ...
        try {
            ...
            if (uinputFd >= 0) {
                ...
                if (!pointerDown && x1 == x2 && y1 == y2) {
                    ...
                    downTime = now;
                    pointerDown = true;
                    MotionEvent down = MotionEvent.obtain(downTime, now, MotionEvent.ACTION_DOWN, 1,
                    ...
                    long tDown = now;
                    MotionEvent down = MotionEvent.obtain(tDown, now, MotionEvent.ACTION_DOWN, 1,
                        new MotionEvent.PointerProperties[]{ptr(touchId)},
                        new MotionEvent.PointerCoords[]{coord(x1, y1)},
                        0, 0, 0.8f, 1f, 0, 0, InputDevice.SOURCE_TOUCHSCREEN, 0);
                    MotionEvent move = MotionEvent.obtain(tDown, now + durationMs, MotionEvent.ACTION_MOVE, 1,
                        new MotionEvent.PointerProperties[]{ptr(touchId)},
                        new MotionEvent.PointerCoords[]{coord(x2, y2)},
                        0, 0, 0.8f, 1f, 0, 0, InputDevice.SOURCE_TOUCHSCREEN, 0);
                    int upDelay = 3 + rng.nextInt(6);
                    MotionEvent up = MotionEvent.obtain(tDown, now + durationMs + upDelay, MotionEvent.ACTION_UP, 1,
                        new MotionEvent.PointerProperties[]{ptr(touchId)},
                        new MotionEvent.PointerCoords[]{coord(x2, y2)},
                        0, 0, 0.8f, 1f, 0, 0, InputDevice.SOURCE_TOUCHSCREEN, 0);
                    injectMethod.invoke(inputManager, down, INJECT_MODE_ASYNC);
